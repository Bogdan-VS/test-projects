...
const Keypad = () => {
  ...
  const sumValue = useCallback(
    value => () => {
      switch (value) {
        case commands.CLEAR:
          dispatch(actionCreatorList.clearCreator())
          break
        case commands.FULL_CLEAR:
          dispatch(actionCreatorList.fullClearCreator())
          break
        case commands.SIGN:
    ...
    [dispatch, initValue, currentExpression, isSign],
  )

  useEffect(() => {
    if (result === errorValue || result === errorExpression) {
      setTimeout(() => {
        dispatch(actionCreatorList.clearResultCreator())
      }, 2000)
    } else if (+result || result === '0') {
      dispatch(actionCreatorList.inputValueCreator(result))
      dispatch(actionCreatorList.clearResultCreator())
    }
  }, [dispatch, result])
  ...
  const btnCollection = BUTTONS_MAP.map(value => {
    return (
      <CalcBtn
        disabled={checkErrorMessage(result)}
        key={value}
        name={value}
